...
module.exports = function(){
    ...
    findAuth: function(criteria, cb){
      var self = this;
			waterlock.Auth.findOne(criteria).exec(function(err,auth){
				if(!err&&auth){
					waterlock.User.findOne({id:auth.user}).exec(function(err,user){
						if(!err&&user){
							auth.user = user;
        ...
        waterlock.User.create({auth:auth.id}).exec(function(err, user){
          ...
          waterlock.Auth.update(auth.id, {user:user.id}).exec(function(err, auth){
            ...
            user.auth = auth.shift();
						waterlock.Auth.findOne({id:uid}).exec(function(err,auth){
							user.auth = auth;
							cb(err,user);
						});
            //cb(err, user);
